...
def bench_one(batch: int, seq: int, dim: int, n_iters: int, warmup: int,
              device: str, fn: Callable) -> dict:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--csv-out", default=None,
                   help="Optional path to write a CSV of (mode, dim, seq, fwd_ms, bwd_ms, mem_MiB, status).")
    p.add_argument("--modes", nargs="+", choices=["eager", "compiled"], default=["eager", "compiled"],
                   help="Which attention variants to benchmark per cell.")
    return p.parse_args()
...
def main() -> None:
    ...
    fn_table = {"eager": attention}
    if "compiled" in args.modes:
        fn_table["compiled"] = torch.compile(attention)
    fns = [(m, fn_table[m]) for m in args.modes]

    print(f"batch={args.batch_size} iters={args.iters} warmup={args.warmup} device={args.device} modes={args.modes}")
    print(f"{'mode':>9} {'dim':>4} {'seq':>6}  {'fwd ms':>9} {'bwd ms':>9} {'mem MiB':>10} {'peak MiB':>10}  status")
    ...
    for dim in args.dims:
        for seq in args.seqs:
            for mode, fn in fns:
                try:
                    r = bench_one(args.batch_size, seq, dim, args.iters, args.warmup, args.device, fn=fn)
                    print(f"{mode:>9} {dim:>4} {seq:>6}  {r['fwd_mean_ms']:>9.3f} {r['bwd_mean_ms']:>9.3f} "
                          f"{r['mem_before_bwd_MiB']:>10.1f} {r['peak_MiB']:>10.1f}  ok")
                    rows.append(dict(mode=mode, dim=dim, seq=seq, status="ok", **r))
                except BaseException as e:
                    if is_oom(e):
                        print(f"{mode:>9} {dim:>4} {seq:>6}  {'-':>9} {'-':>9} {'-':>10} {'-':>10}  OOM")
                        rows.append(dict(mode=mode, dim=dim, seq=seq, status="OOM"))
                        torch.cuda.empty_cache()
                        gc.collect()
                    else:
                        raise
    ...
    if args.csv_out:
        keys = ["mode", "dim", "seq", "status", "fwd_mean_ms", "fwd_min_ms",
